...
def analyze_symbol(symbol: str, df: pd.DataFrame, spread_moves: float | None = None) -> dict:
    ...
    if mode["mode"] == "TREND SELL":
        ...
        if late_moves > NO_CHASE_MOVES:
            ...
            # V2: historical review showed first-breakout entries were weak.
            # Do not enter the first break; wait for pullback or momentum confirmation instead.
            candidates.append({
                "command": "PLAN ONLY — DO NOT ENTER",
                "strategy": "Breakdown Watch — Wait Pullback",
                "direction": "SELL",
                "entry": None, "stop": None, "target": None,
                "risk_moves": None, "reward_moves": None, "rr": None,
                "price": round(price,5),
                "reason": "V2 filter: first breakdown is blocked because review data showed breakdown/breakout entries often reverse. Wait for pullback or momentum confirmation.",
                "market_mode": mode,
                "quality": "WATCH",
            })
        ...
        if late_moves > NO_CHASE_MOVES:
            ...
            candidates.append({
                "command": "PLAN ONLY — DO NOT ENTER",
                "strategy": "Breakout Watch — Wait Pullback",
                "direction": "BUY",
                "entry": None, "stop": None, "target": None,
                "risk_moves": None, "reward_moves": None, "rr": None,
                "price": round(price,5),
                "reason": "V2 filter: first breakout is blocked because review data showed breakout entries often reverse. Wait for pullback or momentum confirmation.",
                "market_mode": mode,
                "quality": "WATCH",
            })
    ...
    # V2 Momentum Confirmation: upgrade repeated PLAN ONLY missed moves into controlled entries.
    # This is intentionally added AFTER pullback logic and AFTER no-chase logic, so it does not chase
    # very late extremes and it does not replace the stronger Pullback Continuation strategy.
    if not [c for c in candidates if c["command"].startswith(("SCALP NOW", "TRADE NOW"))]:
        dist_ema20 = abs(price - ema20) / 0.00001
        dist_ema50 = abs(price - ema50) / 0.00001
        # BUY: bias and trend agree, price is above EMAs, candle confirms, movement is strong but not too extended.
        if mode["bias"] == "BUY" and price > ema20 > ema50 and candle_dir == "BUY":
            ...
            if momentum_ok and structure_ok and not_chasing_high:
                ...
                candidates.append(_trade(
                    "SCALP NOW BUY",
                    "Momentum Confirmation Buy",
                    "BUY", entry, stop, target,
                    "V2 upgrade: previous review showed many BUY SETUP FORMING records became MISSED BUY MOVE. Bias, EMA alignment, candle direction and momentum now confirm entry.",
                    mode, "A-"
                ))
        # SELL: mirror logic for bearish continuation.
        if mode["bias"] == "SELL" and price < ema20 < ema50 and candle_dir == "SELL":
            ...
            if momentum_ok and structure_ok and not_chasing_low:
                ...
                candidates.append(_trade(
                    "SCALP NOW SELL",
                    "Momentum Confirmation Sell",
                    "SELL", entry, stop, target,
                    "V2 upgrade: previous review showed setup-forming records can miss strong continuation moves. Bias, EMA alignment, candle direction and momentum now confirm entry.",
                    mode, "A-"
                ))
