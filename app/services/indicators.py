...
def build(c):
    ...
    return {"price":price,"ema9":e9,"ema20":e20,"ema50":e50,"trend":trend,"rsi":rsi(c),"atr":atr(c),
            ...
            "range_high":max(highs),"range_low":min(lows),"resistance_soft":float(np.quantile(highs,0.82)),
            "support_soft":float(np.quantile(lows,0.18)),"midpoint":float(np.median(closes)),
            "prev_high":max(x["high"] for x in recent[:-3]),"prev_low":min(x["low"] for x in recent[:-3]),"profile":profile(c)}
