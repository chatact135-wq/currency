...
def next_at(h,m=0):
    ...
    return t
def secs(dt): return max(0,int((dt-now()).total_seconds()))
def info():
    n=now(); minutes=n.hour*60+n.minute
    if 16*60+30<=minutes<=20*60: name,score='London/New York Overlap',10
    elif 11*60<=minutes<=20*60: name,score='London Session',6
    elif minutes>=16*60+30 or minutes<=60: name,score='New York Session',6
    elif 2*60<=minutes<=9*60: name,score='Low Liquidity',-8
